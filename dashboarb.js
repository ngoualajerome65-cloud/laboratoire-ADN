...
function loadStats() {
    const stats = LabGeniusStorage.getUserStats();
    ...
    statsGrid.innerHTML = `
        ...
        <div class="stat-card">
            ...
            <div class="stat-details">
                <span class="stat-value">${stats.syntheses}</span>
                <span class="stat-label">Synthèses</span>
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-icon">📊</div>
...
function loadRecentActivity() {
    ...
    if (history.length === 0) {
        recentList.innerHTML = '<p class="empty-message">Aucune activité récente</p>';
        return;
    }
    ...
    recentList.innerHTML = history.slice(0, 5).map(item => `
        <div class="activity-item">
            ...
            <span class="activity-sequence">${item.sequence}</span>
        </div>
    `).join('');
}

function logout() {
    localStorage.removeItem('labgenius_current_user');
